...
echo ""
echo "  ── 복제를 구성해도 단일 노드에서는 시험할 수 없는 것 (§15-2) ──"
gap "분단 내성(split-brain)" "복제의 가장 중요한 실패 모드 — 루프백에서는 분단을 만들 수 없다"
gap "정족수 상실"          "3중 중 하나를 죽이는 것과 노드를 잃는 것은 다르다"
gap "안티어피니티 실동작"  "required 면 스케줄 불가, preferred 면 조용히 뭉친다 — 어느 쪽도 시험이 아니다"
gap "지연 기반 타임아웃"   "루프백 0.1ms 에서 맞춘 값이 실제 네트워크에서 깨진다(동기 복제)"
echo ""
echo "  ★ 단일 노드 시험이 답하는 것은 '설정이 맞는가' 이고"
echo "    답하지 않는 것은 '분단되면 어떻게 되는가' 다. 둘 다 필요하다."
echo ""

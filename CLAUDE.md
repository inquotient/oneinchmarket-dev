...
| [docs/LOCAL-DEPLOYMENT.md](docs/LOCAL-DEPLOYMENT.md) | **브랜치 `local`.** WSL2 단일 노드 k3s + zram (B안). **§8 에 실배포 기록** — WSL2 고유 블로커 3건, 결함 32건 해소. **§9 는 뒤로 미룬 일** — 지금 하지 않기로 **결정한** 목록이다(잊은 것이 아니다). **§3-1 은 콜드 부팅 뒤 체크리스트** — WSL 이 꺼졌다 켜지면 반드시 볼 것. **§23 은 56GB 상한**(§22 의 결론을 뒤집는다) · **§24 는 L0 랩의 KVM 이전**(완료 — Hyper-V 자산은 지웠다) · **§15-6 은 KVM 다중 노드** — H5 가 열려 판단 근거가 바뀌었다. 새 작업을 시작하기 전에 볼 것 |

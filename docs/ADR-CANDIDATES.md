...
- **재검토 (2026-09-05, `LOCAL-DEPLOYMENT.md §10`)**: 기술 위험은 없다(H5 해소, §8-31 에서 12/12 PASS). 이 호스트에서 **WSL 을 종료하면 3노드까지 성립한다**(§10-1-b 실측: 노드당 한계비용 2.82 GiB — DaemonSet requests 0.92 + 시스템 예약 1.9). `L0-Target` 을 끄면 4, requests 를 실사용 기준으로 정정하면 5 까지 가나 **정적 분할(H1)이라 칸이 작아질수록 2 GiB 급 파드의 배치가 위태로워 운용상 권장은 3** 이다. WSL2 기각 근거 중 하나였던 **W3(Falco 기동 불가)는 소멸했다**(§8-64) — 커널이 아니라 이미지가 낡은 것이었다. A안의 남은 근거는 커널 능력이 아니라 **토폴로지**다: 파드 상한(107/110)과 **OPNsense 가 k3s 의 경로에 없다**는 것(§8-34). 이설 비용은 PVC 27개·116 GB 재생성이며 이제 **과금 원장이 포함**된다. 권고 순서는 §10-6 — 가격·인보이스를 먼저 끝내고, `v2` 머지(§9-4)와 함께 계획한다.

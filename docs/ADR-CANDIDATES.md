...
- **재검토 (2026-09-05, `LOCAL-DEPLOYMENT.md §10`)**: 기술 위험은 없다(H5 해소, §8-31 에서 12/12 PASS). 그러나 이 호스트에서는 **2노드만 성립하고 3노드는 들어가지 않는다** — 워크로드 requests 가 35.16 → 38.9 GiB 로 늘었고 L0 랩 8.8 GiB 를 뺀 Windows 몫이 3노드에서 2.2 GiB 다. WSL2 기각 근거 중 하나였던 **W3(Falco 기동 불가)는 소멸했다**(§8-64) — 커널이 아니라 이미지가 낡은 것이었다. A안의 남은 근거는 커널 능력이 아니라 **토폴로지**다: 파드 상한(107/110)과 **OPNsense 가 k3s 의 경로에 없다**는 것(§8-34). 이설 비용은 PVC 27개·116 GB 재생성이며 이제 **과금 원장이 포함**된다. 권고 순서는 §10-6 — 가격·인보이스를 먼저 끝내고, `v2` 머지(§9-4)와 함께 계획한다.

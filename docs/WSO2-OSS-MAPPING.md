...
| **Pyrra / Sloth**(SLO) | 보류 | **진짜 빈칸이다** — Prometheus 규칙이 그룹 2 · alert 5 · **record 0** 이고 SLO CRD 0건. 다만 SLO 는 "무엇의 가용성인가" 가 먼저다. 외부 진입 경로가 **ADR-079 미결**이라 대상이 흔들리는 상태에서 박으면 에러버짓 이력이 함께 흔들린다(과금에서 겪은 Gotcha 15 와 같은 부류) | 게이트웨이 경로가 정해질 때. ★ 그때도 **Sloth 를 먼저** — 규칙 YAML 을 생성해 git 에 두는 방식이라 `render-ratelimit.py --check` 와 같은 모양이고, Pyrra 는 상주 컴포넌트+UI 라 84% 노드에서 비싸다 |
...
| **OpenSearch** | 제외 — **하드 블로커는 수집기이고, 기능 티어는 실재하는 반대 논거다**(2026-09-11, 같은 날 한 번 정정했다) | ★ **막는 것: Beats ≥ 7.13 은 OpenSearch 를 지원하지 않는다(7.12.1 이 마지막).** 이 클러스터는 **Filebeat 9.5.3** 이 `output.elasticsearch` 로 ES 에 **직접** 쓰고, 그 데이터가 전체의 거의 전부다 — 실측 26 인덱스 31.66 GB 중 `.ds-filebeat-*` 6개가 약 32 GB(최대 하나 160,732,387 docs · 17.2 GB). 즉 전환은 엔진 교체가 아니라 **수집기 교체**다(①7.12.1 로 강등 ②filebeat→Logstash→`logstash-output-opensearch` ③Fluent Bit·Data Prepper). 그 위에 Kibana→OpenSearch Dashboards · ILM→ISM 재작성 · **ES 9(Lucene 10.5.1) 스냅샷은 OpenSearch 에 복원되지 않아** 31.66 GB 재수집. 나머지 소비자 둘(Logstash 9.5.3 · Falcosidekick 2.35.0)도 `elasticsearch` 출력이다 | ★★ **끄는 것: 기능 티어 — 이 칸은 한 번 틀리게 적혔다.** "Elastic 이 AGPL 을 더했으니 명분이 사라졌다" 로 썼는데 **소스 라이선스와 기능 티어는 다른 이야기**다. 실측 표: **SAML/OIDC/LDAP 렐름 · DLS · FLS·필드 마스킹 · 감사 로그 · IP 필터링이 전부 Enterprise**, Watcher·ML 은 Gold+ 인데 **Gold 단종 · Platinum 기존 고객 전용**이라 경로가 Enterprise 하나다. OpenSearch 는 같은 것들을 security 플러그인에 **무료로** 담는다. 즉 포크의 두 이유 중 **라이선스만 약해졌고 기능 게이팅은 오히려 나빠졌다.** ★ **그런데 우리가 지금 그것들을 쓰지 않는다**(이것이 실제 근거다): Kibana 는 SSO 없이 `elastic` 슈퍼유저로 붙고 port-forward 전용이며, **SEC-205 의 계획이 애초에 `oauth2-proxy + Keycloak`** 이라 OIDC 를 ES *안*이 아니라 *앞*에 둔다(Basic 에서 된다). 감사는 **SEC-307 이 Wazuh 로** 보내는데 그 indexer 가 이미 OpenSearch 다. DLS/FLS 는 테넌트(`acme-corp`·`globex-corp`)가 실재하지만 **ES 를 직접 질의하는 테넌트가 0** 이다(`api-usage-dlq`·`api-usage-quarantine` 는 우리가 읽는 운영 인덱스다). ★★★ **복귀 조건 둘**: ⓐ Kibana 를 oauth2-proxy 없이 Keycloak 에 직접 물려야 할 때, 또는 감사 로그·DLS/FLS 를 **검색 계층 자체에서** 요구받을 때 — 그때 Elastic 의 답은 Enterprise 계약이다 ⓑ Filebeat 을 다른 이유로 걷어낼 때(유일한 하드 블로커가 사라진다). ★ 그리고 **지금 아픈 곳은 엔진이 아니다**(§9-16): 힙 1 GB 에 데이터 31.66 GB·샤드 77 · 파드 재시작 20회 · 17.2 GB 가 고아 데이터 스트림에 갇혀 있다 |
| **BuildKit / Kaniko** | 보류 | 빌드 경로가 **이미 둘**이다 — 호스트 podman 11종, Camel K 는 **클러스터 안에서** 빌드한다(`strategy=pod`·`publish=Jib`). ★ CI 에는 이미지 빌드 잡이 **아예 없다**(잡 14개). 주석이 이유까지 적어 뒀다 — `docker:dind` 는 privileged 가 필요한데 이 러너는 주지 않는다 | **CI 에서 이미지를 굽는 것이 실제 요구가 될 때.** 다만 그 전에 `inquotient/admin`·`cmmn-api` 의 **소스가 이 레포에 없다**는 것부터 풀어야 한다 |
| **Paketo Buildpacks** | 보류 | `docker/` 의 11종은 대부분 **업스트림을 패치해 굽는 것**이다(Ranger Jersey 포팅 · HBase 3 이식 · Backstage 앱). Buildpacks 는 "앱 소스 → 이미지" 를 자동화하는 도구라 이런 패치형 빌드에 맞지 않는다. 맞을 대상(`admin`·`cmmn-api`)은 **소스가 레포 밖**이다 | 앱 소스가 이 레포로 들어올 때 |

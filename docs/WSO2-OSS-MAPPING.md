...
|---|---|---|---|
| **Apache Flink** | ~~보류~~ → **✅ 도입 (2026-09-12, 사용자 결정 "일단 도입해줘")**. ★★ **복귀 조건이 충족되어 넣은 것이 아니다** — 조건은 "Kafka Streams 로 손수 짠 패턴 매칭이 셋을 넘을 때" 인데 실측은 **0건**이었다(레포 참조 0 · 스트림 워크로드 0). 즉 지금 **일감이 0건**이고, KEDA·Crossplane·kpack 과 같은 상태다. ★★★ **다만 보류 사유의 한 기둥은 넣기 전에 이미 무너져 있었다** — 아래 "Iceberg 적재는 Kafka Connect 로 된다" 는 **거짓이다**: Gotcha 116 이 그 근거(Central 의 1.11.0)를 정정했고, 실측 Connect 플러그인 4종은 Debezium·MirrorMaker 뿐이며 **지금 Kafka → Iceberg 를 하는 것은 0개다.** 그래서 Flink 의 첫 일감 유력 후보가 바로 그것이다. ★ 비용 실측: requests **2,560Mi**(JM 1,024 + TM 1,536), 노드 예약 88% → **93%**. 걷어내려면 `base/kustomization.yaml` 의 `- streaming/` 한 줄을 지운다. ★ 넣으며 결함 둘을 고쳤다(Gotcha 164·165): **TM 의 rpc·data 포트가 기본 랜덤**이라 선언한 6122 를 아무도 듣지 않아 TM 이 영원히 0/1 이었고(로그는 `Successful registration` 으로 정상, 단서는 kubelet 이벤트뿐), **`limits.cpu` 가 없어** Kyverno `require-resource-limits` 가 fail 했다(local 은 Audit 이라 통과 — prod 였으면 거부). 고친 뒤 판정: JM `/overview` = `{"taskmanagers":1,"slots-total":2,"slots-available":2}` | 스트림 잡 **0건**. 배치는 Spark 가 이미 한다. Iceberg 적재는 `org.apache.iceberg:iceberg-kafka-connect` **1.11.0** 이 Maven Central 에 있어 Kafka Connect 로 된다(클러스터의 `ICEBERG_VERSION` 과 같은 값). 윈도우·조인·간단한 CEP 는 Kafka Streams 로 된다 — **라이브러리라 인프라가 0이다** | Kafka Streams 로 손수 짠 패턴 매칭이 셋을 넘거나, **이벤트 타임 + 워터마크 + 큰 상태**가 필요해질 때. ★ 매니페스트는 `kubernetes/base/streaming/` 에 이미 있으나 **`base/kustomization.yaml` 에 연결하지 않았다** — 되살릴 때 그 한 줄부터 |
...
| **Coraza + CRS** | 제외 | 사용자 결정(2026-09-10). ★ 검토 중 함께 드러난 것이 더 중요하다 — **SafeLine 이 트래픽 경로 밖이다**: HTTPRoute backend **0건** · IngressClass **0건**인데 **1.47 GiB** 를 예약하고 자기 로그에 `502 Bad Gateway` 를 찍고 있다 | ~~**WAF 칸은 Coraza 도입이 아니라 SafeLine 정리가 먼저다.** 경로에 넣든 걷어내든 하나를 고를 것~~ → **2026-09-12 에 "경로에 넣는다" 로 정리했다(사용자 결정).** 배치는 Gravitee 와 같은 원칙이다: `바깥 → **Istio** → SafeLine → cmmn-api` — Istio 가 먼저라야 계량 지점이 움직이지 않는다(Gotcha 15). 실측: 토큰 없이 **403** · 토큰과 함께 **200** · SQLi/XSS/경로순회 **403** · 액세스 로그에 `route=local.api.0` 이 status 별로 그대로 남는다. ★★ 넣는 과정에서 셋이 드러났다(Gotcha 163): ① 사이트 생성이 포트 충돌로 **조용히 실패**했다(`err` 는 비고 `msg` 에만 `address already in use`) — 한 파드가 네트워크 네임스페이스를 공유해 :80 은 mgt, :8080 은 chaos 가 쓰고 있었다. 8181 로 옮겼다 ② `worker_processes` 가 auto 라 nginx 가 **25 프로세스 · 1799Mi** 였다. postStart 훅으로 2개 고정 → **3 프로세스 · 161Mi**(11배). 예약도 1024→384Mi 로 정직해졌다 ③ **WAF 차단이 과금 스트림에 403 으로 들어온다** — 전까지 403 은 "토큰 없음" 뿐이었다. ★ 사이트 정의는 mgt 의 DB 에만 살아 `local/safeline-sites/*.json` + `local/safeline-bootstrap.sh` 로 git 에 옮겼다(Gotcha 159). 로그인이 평문을 받지 않아 그 계약을 **SPA 번들에서 읽어냈다**(Gotcha 162). ★★ **남은 구멍**: `/managed`(Gravitee) 경로는 아직 WAF 밖이다 — Gravitee 엔드포인트를 SafeLine 으로 돌려야 하는데 두 부트스트랩이 **생성 전용이라 갱신을 못 하고**, Gravitee 가 어떤 Host 를 보내는지도 미측정이다(SafeLine 사이트는 `server_names` 로 매칭한다). **복귀 조건 — `/managed` 에 시험용이 아닌 실제 API 가 실리는 날.** 그때 두 스크립트에 갱신 경로를 먼저 넣을 것. Coraza 는 여전히 제외다: WAF 칸이 이제 비어 있지 않다 |

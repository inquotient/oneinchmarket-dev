...
| **Harbor** | 제외 | GitLab 컨테이너 레지스트리가 **이미 돈다**(`gitlab-registry` 80/5050). 2026-09-11 에 `registries.yaml` 을 고쳐 **pull 이 처음으로 되게** 만들었다(Gotcha 126). 둘째 레지스트리는 이미지 원천을 둘로 만든다 | GitLab 을 걷어내기로 할 때 |
| **OpenSearch** | 제외 | 검색 엔진이 **이미 둘**이다 — Elasticsearch(ECK)+Kibana, 그리고 **Wazuh indexer 는 OpenSearch 계열**이다. 셋째를 들이는 것이 아니라 **둘을 줄이는 쪽**이 값이 크다 | ES 라이선스나 ECK 가 제약이 될 때. ★ 그 전에 Wazuh indexer 와의 중복 정리가 먼저다 |

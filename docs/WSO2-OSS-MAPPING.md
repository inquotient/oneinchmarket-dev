...
| **OpenSearch** | 제외 — **전환을 실제로 검토했고 하드 블로커가 하나 있다**(2026-09-11) | ★ **Beats ≥ 7.13 은 OpenSearch 를 지원하지 않는다 — 7.12.1 이 마지막이다.** 이 클러스터는 **Filebeat 9.5.3** 이고, 그것이 `output.elasticsearch` 로 **ES 에 직접 쓴다.** 그리고 그 filebeat 데이터가 전체의 거의 전부다 — 실측 26 인덱스 31.66 GB 중 `.ds-filebeat-*` 6개가 **약 32 GB**(최대 하나가 160,732,387 docs · 17.2 GB). 즉 전환은 "엔진 교체" 가 아니라 **수집기 교체**다: ① filebeat 을 2021년판 7.12.1 로 내리거나(보안 패치 없는 DaemonSet) ② OpenSearch 가 안내하는 우회인 filebeat→Logstash→`logstash-output-opensearch` 를 태우거나(지금 Logstash 9.5.3 에는 `logstash-output-elasticsearch` 만 있다) ③ Fluent Bit·Data Prepper 로 갈아탄다. 그 위에 Kibana→OpenSearch Dashboards · ILM→ISM 재작성 · **ES 9(Lucene 10.5.1) 스냅샷은 OpenSearch 에 복원되지 않아** 31.66 GB 를 reindex 하거나 재수집해야 한다. 나머지 소비자 둘(Logstash · Falcosidekick 2.35.0)도 전부 `elasticsearch` 출력이다 | ★★ **옛 복귀 조건의 절반이 소멸했다.** 전에는 "ES 라이선스나 ECK 가 제약이 될 때" 로 적었는데, **라이선스 쪽은 없어졌다** — Elastic 이 2024-08 에 **AGPL 을 ELv2·SSPL 옆에 추가**했고(빼지 않고 더한 것이다) 이 클러스터는 실측 `"type":"basic"`(무료)으로 돈다. OpenSearch 포크를 낳은 이유 자체가 사라진 것이다. 남는 조건은 **"Filebeat 을 어차피 걷어낼 때"** 하나다 — 유일한 하드 블로커가 그것이므로, 다른 이유로 수집기를 바꾸는 날 전환 비용이 급감한다. ★ **그리고 지금 아픈 곳은 엔진이 아니다**(§9-16): 힙이 **1 GB** 인데 데이터 31.66 GB · 샤드 77개이고 ES 파드가 **20회 재시작**(exit 255)했으며, 17.2 GB 는 **쓰는 이가 없어 영원히 롤오버되지 않는** 고아 데이터 스트림이다. 엔진을 바꿔도 이 셋은 그대로다 |

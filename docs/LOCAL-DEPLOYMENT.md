...
- ~~요금제와 한도를 잇는 자동화가 없다~~ — **§8-90 에서 막았다.**
  `local/pricing-catalog.yaml` 이 단일 원천이고 쿼터는 거기서 렌더된다
- `x-ratelimit-*` 헤더는 `DRAFT_VERSION_03` 이다. 표준이 바뀌면 소비자 쪽
  파싱이 깨질 수 있다

### 8-90. 요금제를 테스트 데이터로 채웠다 — 그리고 쿼터를 같은 원천에서 뽑는다 (2026-09-08)

§8-89 가 남긴 구멍을 막는다: *"요금제와 한도를 잇는 자동화가 없다 — 요금제를
바꾸면 사람이 두 곳을 고쳐야 하고, 어긋나도 아무도 알려주지 않는다."*
**"Pro 를 샀는데 Free 한도로 막힌다" 는 조용히 생긴다.**

#### 원천을 하나로 모았다

```
local/pricing-catalog.yaml            ← 가격과 쿼터의 단일 원천 (테스트 데이터)
   ├─ local/openmeter-pricing.sh      → OpenMeter plan · customer · subscription
   └─ local/render-ratelimit.py       → kubernetes/base/gateway-ratelimit/
                                          ratelimit-config.yaml (ConfigMap)
```

★ 렌더 결과를 **git 에 커밋한다.** ArgoCD 가 그것을 적용하므로, 카탈로그만
고치고 렌더를 잊으면 쿼터가 그대로인데 **git diff 에 드러난다.**
`render-ratelimit.py --check` 는 뒤처졌으면 1 을 돌려주므로 CI 게이트로 쓸 수 있다.

| 요금제 | 월 구독료 | 요청당 | 쿼터 |
|---|---:|---:|---:|
| `oim_api_free` | $0 | $0 | 60/분 |
| `oim_api_standard` | $20 | $0.001 | 600/분 |
| `oim_api_pro` | $199 | $0.0005 | 3000/분 |

★★ **전부 테스트 데이터다.** 가격은 소급 적용이 가능하지만 청구 이력은
불가능하다(Gotcha 15) — 실제 고객 전에 카탈로그를 실제 값으로 바꿔야 한다.
★ 카탈로그에 없는 테넌트는 60/분, 토큰에 tenant 클레임이 없으면 10/분이다 —
**무제한이 되는 경로를 남기지 않으려는 것**이다.

#### 스크립트가 드리프트를 조용히 고치지 않는다

globex 를 `pro` 로 옮기려 했더니 스크립트가 이렇게 말했다:

```
구독 있음: 01M1Y3YX5... plan=oim_api_standard   ** 카탈로그(oim_api_pro)와 다르다
```

**일부러 자동으로 바꾸지 않는다** — 구독 변경은 청구 경계를 움직이기 때문이다
(아래에서 실제로 그 대가를 봤다). 옮기는 것은
`POST /api/v1/subscriptions/{id}/change` 로 명시적으로 한다.

#### 검증 ① 테넌트별 쿼터가 요금제대로 갈린다

헤더 한 줄이면 판정된다 — 3000회 버스트가 필요 없다.

```
acme-corp   -> x-ratelimit-limit: 600, 600;w=60    (standard)
globex-corp -> x-ratelimit-limit: 3000, 3000;w=60  (pro)
```

#### 검증 ② 인보이스에 의미 있는 금액이 찍힌다

`local/seed-usage.py` 로 테넌트당 11,000건을 넣었다(5xx 10% 포함).
계약(`contracts/schemas/api-usage-event.json`)을 그대로 따르고, Kafka 과금
토픽이 아니라 **ingest API** 로 보낸다(Gotcha 23).

| | 계량 전체 | 청구대상(<500) | 인보이스 qty | 합계 |
|---|---:|---:|---:|---:|
| acme-corp (standard) | 11,045 | 9,943 | **9,903** | **$29.90** |
| globex-corp (pro) | 11,004 | 9,904 | **5,891** | **$201.95** |

5xx 는 1,102 / 1,100 건이 계량되고 **한 건도 청구되지 않았다** — §8-87 의
feature 필터가 규모에서도 동작한다.

#### ★★ 차이는 전부 구독 경계다 — 그리고 이것이 이 절의 핵심 교훈이다

계량과 청구가 정확히 일치하지 않는데, **둘 다 맞다.**

- **acme 40건** — 구독 시작(14:17) **이전**의 사용량이다. §8-87 에서 관찰한
  "구독 이전 사용량은 청구되지 않는다" 가 규모에서도 그대로다.
- **globex 4,013건** — 요금제를 바꾼 시각(17:43:30) **이전**의 사용량이다.
  요금제 변경은 구독을 **새로 만든다**:

```
01M1Y3YX5... inactive  activeFrom=14:22:08  activeTo=17:43:30   (standard)
01M1YFFME... active     activeFrom=17:43:30  activeTo=None       (pro)
```

★ **여기서 조심할 것** — 경계 이전 사용량은 **옛 구독**의 인보이스에 속한다.
실측에서 옛 구독의 인보이스는 `draft · total=0 · qty=0` 이었다. 즉 요금제를
바꾼 **뒤에 도착한** 그 기간의 사용량은 어느 인보이스에도 실리지 않았다.
이 랩에서는 시드 데이터라 문제가 아니지만, **운영에서는 매출 누락**이다.
계량이 늦게 도착하는 경로가 있다면(DLQ 재처리 §8-62 가 그렇다) 요금제 변경
시각 근처에서 유실이 생길 수 있다 — §9-2 에 넣었다.

#### 남는 것

- **인보이스 공급자가 여전히 "OpenMeter"(US)** 다(§9-2). 발행 주체를 정하는
  일이라 사업자 정보가 필요하다
- **요금제 변경 경계의 늦게 도착한 사용량** — 위 문단. 관찰만 했고 처방은 없다
- 카탈로그의 `unknownTenantQuotaPerMinute` 는 쿼터에만 쓰인다 — **구독 없는
...
### 8-102. 남은 도입 — 산술이 하나만 허락했다. OpenFGA (2026-09-08)

`WSO2-OSS-MAPPING.md` 의 도입 목록을 실제로 채우려다 두 가지를 했다:
**낡은 서술을 사실로 바꾸고**, 그 뒤에 **들어갈 수 있는 것 하나**를 넣었다.

#### ① 문서가 도입을 막고 있었다 — 이미 끝난 것을 안 끝났다고 적어서

| 항목 | 문서가 말한 것 | 실측 (2026-09-08) |
|---|---|---|
| Phase 0 ① 시크릿 | "Vault 는 `0/1`(봉인)이고 시크릿 관리는 미작동" | **OpenBao `2/2 Running` · `Sealed=false` · ExternalSecret 29건 전부 `SecretSynced`** |
| Phase 0 ② ArgoCD | "**CRD 0 · 파드 0 · 네임스페이스 없음**" | **네임스페이스 · 파드 7 · Application 1 · 자동 동기화 중** |
| Phase 1 ④ 쓰로틀링 | "요금제와 한도를 잇는 자동화는 아직 없다" | §8-90 에서 생겼다(`pricing-catalog.yaml` 단일 원천) |
| Phase 1 ⑤ 계약 | "지금 **0건**" | 1건(§8-99) |
| Phase 1 ⑥ 요금제 | 미완 | §8-87·§8-90 에서 끝났다 |

★ **낡은 계획서는 계획을 막는다** — Phase 0 이 "깨져 있다" 고 적혀 있으면
Phase 2 로 갈 수 없다고 읽힌다. 도입 작업의 절반은 문서를 사실로 되돌리는
것이었다.

#### ② 그러고 나서 남은 것을 세어 보니, 산술이 하나만 허락했다

| 남은 도입 | §7 추정 | 들어가나 |
|---|---|---|
| **OpenFGA** | 0.15 ~ 0.25 GiB | **된다** |
| Flowable / Temporal | 0.5 ~ 0.8 | 빠듯 |
| Camel K | 0.5 ~ 1.0 | 빠듯 |
| Backstage | 0.7 ~ 1.0 | 빠듯 |
| midPoint | 1.0 ~ 1.5 | 안 된다 |
| Debezium | 1.0 | 안 된다 |
| Gravitee CE | 2.0 ~ 2.5 | 안 된다 |
| Flink | 2.0 ~ 3.0 | 안 된다 |
| **합계** | **8 ~ 14 GiB** | 노드 여유 **1.2 GiB** |

문서 자신이 적어 둔 그대로다 — **"Phase 0 의 프로파일 분리가 선행되지
않으면 들어갈 자리가 없다. 이것은 취향이 아니라 산술이다."**

#### ③ OpenFGA 를 고른 근거 — "둘 다가 아니라 하나"

문서는 인가 칸을 "OpenFGA 또는 OPA. **둘 다가 아니라 하나**" 로 못박았다.
이 플랫폼에는 **규칙 기반 정책 엔진이 이미 둘** 있다:

| | 무엇을 판단하나 |
|---|---|
| Kyverno | 어드미션 — "이 매니페스트를 받아도 되나" |
| Istio AuthorizationPolicy | 서비스 간 — "이 SA 가 이 포트에 붙어도 되나" |
| **비어 있던 칸** | **애플리케이션 데이터** — "이 사용자가 이 문서를 고쳐도 되나" |

세 번째는 규칙이 아니라 **관계**로 표현된다("편집자인가"). ReBAC 쪽이 맞고,
OPA 를 더하면 규칙 엔진이 셋이 된다.

#### 배선 — 하나라도 빠지면 조용히 깨지는 것들

- **전용 SA**. ambient 에서 SA 는 곧 신원이라 `default` 로는 정책을 쓸 수 없다(Gotcha 10)
- **`allow-database-access` 의 5432·3307 양쪽**에 `*/sa/openfga`.
  없으면 ALLOW 정책이 이 워크로드를 선택한 뒤 매칭되지 않아 **전면 거부**된다(Gotcha 9)
  ★ `*/sa/openmeter` 가 **5곳**(5432·3307·6379·8123/9000·9092·9200)에 있어
  일괄 치환하면 엉뚱한 포트가 열린다. 두 곳만 짚었다
- **postgres-bootstrap** 이 `openfga` 롤·DB 를 만든다(멱등)
- **마이그레이션은 initContainer**. 별도 Job 이면 wave 안에서 순서가 보장되지 않는다
- `create-secrets.sh` 에 항목 추가 + OpenBao 이관 + ExternalSecret 등록.
  ★ `datastore-uri` 안에 비밀번호가 들어가므로 **`db-password` 와 같은 값이어야
  한다** — postgres-bootstrap 은 전자로 롤을 만들고 OpenFGA 는 후자로 붙는다

#### 상류 기본값과 다르게 잡은 둘

| 설정 | 상류 | 여기 | 왜 |
|---|---|---|---|
| `OPENFGA_PLAYGROUND_ENABLED` | true | **false** | 인증 없이 저장소를 조작할 수 있다 |
| `OPENFGA_AUTHN_METHOD` | none | **preshared** | `none` 이면 메시 안 누구나 인가 결정을 바꾼다 — **인가 서버가 인가되지 않는 상태**가 된다 |

#### 검증 — 파드가 아니라 인가로

```
① 토큰 없이 GET /stores            -> 401      (인증이 실제로 강제된다)
② store 생성                        -> id 발급
③ 인가 모델(editor/viewer) 작성     -> model id 발급
④ user:alice = editor of document:1 -> 200
⑤ check:
     alice editor document:1 -> True
     alice viewer document:1 -> True   ← 쓴 적 없다. union 규칙으로 추론된 것
     bob   viewer document:1 -> False
⑥ store 삭제                        -> 204   (탐침 정리)
```

★ ⑤의 두 번째 줄이 이 도입의 요점이다 — 관계를 하나 쓰면 파생 권한이
따라온다. 그것이 ReBAC 이고, Kyverno·Istio 정책으로는 표현할 수 없다.

#### ★★ 그리고 §7 의 추정표를 실측으로 갈았다

```
OpenFGA 추정 0.15~0.25 GiB  →  실측 17Mi
```

**10분의 1이다.** 문서가 "도입할 때마다 `kubectl top pod` 로 실측해 이 표를
교체할 것" 이라고 적어 둔 이유가 이것이다. 다만 **이것을 근거로 합계를 낮춰
...
| ~~**실제 가격**~~ | **테스트 데이터로 진행하기로 정했다**(2026-09-08). 3단 요금제가 `local/pricing-catalog.yaml` 에 있고 쿼터도 같은 파일에서 나온다(§8-90) | 실제 고객 전에 그 파일을 실제 값으로 바꾼다. 가격은 소급 적용이 가능하지만 **청구 이력은 불가능하다**(Gotcha 15) |
| ★ **요금제 변경 경계의 늦은 사용량** | 요금제를 바꾸면 구독이 새로 생기고, 경계 **이전** 사용량은 옛 구독에 속한다. 실측에서 옛 구독 인보이스는 이미 `draft·qty=0` 이라 **늦게 도착한 그 기간의 사용량이 어느 인보이스에도 실리지 않았다**(§8-90) | 관찰만 했다. DLQ 재처리(§8-62)처럼 늦게 도착하는 경로가 있으면 **운영에서는 매출 누락**이다 |
| **구독 없는 테넌트** | 게이트웨이는 유효한 토큰이면 통과시키므로(§8-89) 구독 없이 API 를 쓰는 경로가 열려 있다 — 쿼터는 걸리지만 **청구되지 않는다** | 미착수 |
...
### 9-9. 저장 시 암호화(TDE) — **미루기로 결정했다** (2026-09-07)

#### 이것은 잊은 것이 아니라 미룬 것이다

v1 에는 MariaDB TDE 가 있었고 **v2 로 오며 사라졌다.** `docs/COMPONENTS.md` 의
G22 가 그것을 기록하고 있다:
...
G22 의 **"초기화" 절반은 §8-76 에서 `mariadb-bootstrap` 으로 고쳤고, "TDE"
절반은 그대로 남아 있다.** 실측으로도 저장 시 암호화가 설정된 컴포넌트가 0건이다.

#### KMIP 서버는 필요 없다

이 질문이 먼저 나왔고 답은 "필요 없다" 다. 근거는
[WSO2-OSS-MAPPING.md 부록 A-5](WSO2-OSS-MAPPING.md) 에 표로 있다 — 요지는
**envelope 암호화를 할 수 있는 둘(Percona MongoDB · MinIO)이 모두 Vault
프로토콜을 네이티브로 말한다**는 것이다. OpenBao 가 그대로 받는다.
MariaDB 커뮤니티는 애초에 KMIP 를 못 쓴다(Enterprise 전용).

#### 채울 때의 순서 — 되돌릴 수 있는 것부터

OpenBao 가 섰으므로(§8-80~82) 경로는 셋 다 열려 있다. **순서가 중요하다:**

| 순서 | 대상 | 방법 | 되돌리기 |
|---|---|---|---|
| 1 | **MinIO** | SSE → KES → OpenBao | **쉽다** — 기존 객체를 재암호화하지 않아도 된다(신규부터 적용) |
| 2 | **MongoDB(Percona)** | 네이티브 Vault 키 관리 | 중간 — 암호화는 **초기화 시점**에 정해져 기존 데이터가 있으면 덤프·복원이 필요하다 |
| 3 | **MariaDB** | `file_key_management` 키를 ESO 로 배달 | **어렵다** — 기존 테이블을 재암호화해야 하고, 키를 잃으면 데이터를 잃는다 |

★ **3번을 먼저 하지 말 것.** 키 관리가 검증되지 않은 상태에서 기존 데이터를
암호화하면, 키가 어긋났을 때 되돌릴 방법이 없다. 1번으로 "OpenBao 에서 키를
받아 쓰는 경로" 를 먼저 증명하고 올라가는 것이 맞다.

★★ 그리고 **키를 잃으면 데이터를 잃는다.** OpenBao 자체의 백업(§8-81 의 raft
스냅샷)이 먼저 신뢰할 수 있어야 한다 — 지금 스냅샷은 뜨지만 **복구는 검증하지
않았다**(부록 A-2). TDE 를 켜기 전에 그 검증이 선행되어야 한다.

#### 지금 미루는 이유

- 이 랩의 위협 모형에서 저장 시 암호화의 이득이 크지 않다. §11-0 이 **디스크·
...
### 9-14. ~~Trivy 가 이미지를 인터넷에서 다시 받는다~~ — **①로 해소했다 (2026-09-11)**
...
#### 해소 — ①을 골랐고, 세 칸을 동시에 채워야 했다 (2026-09-11)

표의 ①에 "확인이 먼저다" 라고 적어 둔 것을 **소스로 확인했다**: trivy-operator
v0.34.0 에는 `scanJob.customVolumes`·`scanJob.customVolumesMount` 가 실재하고
(`pkg/trivyoperator/config.go`), `builder.go` 가 그것을 **모든 스캔 컨테이너와
초기화 컨테이너에** 붙인다. `trivy.configFile` 도 있어 `--config` 가 자동으로 붙는다.

그런데 실제로 돌리자 **칸이 셋이었다**(Gotcha 133):

| 칸 | 왜 막혔나 | 어디서 채웠나 |
|---|---|---|
| ① 소켓 | `CONTAINERD_ADDRESS` 는 env 전용 | `scanJob.customVolumes` + **마운트 경로를 기본값에 맞춤** |
| ② 네임스페이스 | `CONTAINERD_NAMESPACE` 도 env 전용이고 오퍼레이터에 손잡이가 없다 | **Kyverno mutate**(`local/trivy-scanjob-containerd.yaml`) |
| ③ 이미지 이름 | containerd 는 `docker.io/…` 로 저장하는데 trivy 는 **짧은 이름으로 export** 를 건다 | 매니페스트 **126줄 / 93파일** 완전지정 |

★★ ③을 빼면 ①②는 **개선이 아니라 회귀**다 — 소켓이 붙으면 이미지 객체가 만들어진
뒤에 실패하므로 원격 폴백이 작동하지 않는다.

**실측**

```
짧은 이름   graviteeio/apim-gateway:4.12.19            -> 0초 만에 not found
완전지정    docker.io/graviteeio/apim-gateway:4.12.19  -> 429초 로컬 읽기, 망 오류 0
롤아웃      Synced/Healthy · 파드 134 · 비정상 0 (약 10분, 이미지가 로컬이라 pull 없음)
스캔 실패   직전 25분 37건 -> 새 이름 뒤 20분 **0건**
```

★ 비용 구조가 바뀌었다 — 원격은 "받다 끊기는" 것이 문제였고 로컬은 **푸는 데
시간이 든다**(745MB 에 429초). `trivy.timeout` 20m · Job 25m · 스캔 cpu 2000m.

★ 남은 예외 하나는 원인이 다르다 — **블롭이 없는 이미지**(Gotcha 134).
`k3s ctr -n k8s.io content fetch <ref>` 로 채운다.

★ 판정은 여전히 **건수가 아니라 본 컨테이너에 리포트가 있는지**다
(Gotcha 53 ★ · 130). 이름이 바뀌며 ReplicaSet 이 전부 새로 생겨 리포트를
...
### 9-16. ~~ES 가 아픈 이유 셋~~ — **OpenSearch 전환으로 정리됐다 (2026-09-11)**

> **해소 기록.** 아래 진단은 전환 **전**의 Elasticsearch 상태다. 같은 날
> 사용자 결정으로 OpenSearch 3.8.0 + Data Prepper 2.16.0 으로 갈아탔고
> (WSO2-OSS-MAPPING §9-1), 셋 중 ①②는 데이터를 옮기지 않기로 하면서
> 함께 사라졌다 — 새 클러스터에는 고아 데이터 스트림도, 31.66 GB 도 없다.
> ③(검색 엔진 둘)은 **그대로 남아 있다**: OpenSearch(로그·SIEM)와
> Wazuh indexer(보안 이벤트)다. 다만 이제 둘이 **같은 계열**이라
> 합칠 때 기능을 버릴 필요가 없어졌다 — 아래 ③의 단서가 그 이유를 적어 뒀다.
>
> ★ 새 스택에서 다시 볼 것: 힙은 1 GB 로 같다(limit 2Gi). 데이터가 늘면
>   **힙과 limit 을 함께** 올려야 한다(Gotcha 82). ISM 정책 `oim-retention`
>   이 14일 뒤 삭제하므로 ①의 무한 증가는 구조적으로 막혀 있다.
> ★★ **감사 로그·DLS/FLS·SAML/OIDC 렐름은 이제 켤 수 있다**(OpenSearch
>   security 플러그인에 무료로 들어 있다 — Elastic 은 전부 Enterprise).
>   지금은 `plugins.security.audit.type: noop` 으로 꺼 두었다. 켤 조건은
>   SEC-3xx 가 검색 계층 감사를 요구할 때이고, 그때 `internal_opensearch`
>   로 바꾸면 된다.

#### (아래는 전환 전의 진단 — 보존)

OpenSearch 전환을 검토하며 ES 를 실측했더니, 전환 여부와 무관한 결함이 셋 나왔다.
**엔진을 바꿔도 셋 다 그대로 따라온다.**

```
ES 9.5.3 · 단일 노드 · status=green · 인덱스 26개 · 31.66 GB · 샤드 77개
heap.max = 1gb   (limits=requests=2Gi)      heap 사용률 73% · ram 90%
파드 재시작 20회 · lastState exit=255
```

**① 힙 1 GB 로 31.66 GB · 샤드 77개를 서빙한다.** Gotcha 82 가 경계하는 것의
반대 방향이다 — 힙이 limit 과 같아서 위험한 게 아니라, **데이터 대비 힙이 작다.**
샤드 하나당 힙 소요가 있고 77개면 그것만으로도 빠듯하다. 재시작 20회가 그 결과로
보인다(exit 255 는 사유를 말해 주지 않는다 — Gotcha 81 과 같은 부류로, 판정하려면
컨테이너 안에서 `_nodes/stats/jvm` 의 GC 시간을 봐야 한다).

**② 17.2 GB 가 영원히 지워지지 않는 고아 데이터 스트림에 있다.** 실측:

```
filebeat-9.5.2  백킹인덱스 1개  ilm_policy=filebeat
  .ds-filebeat-9.5.2-2026.09.03-000005
    docs 160,732,387 · 17.2 GB
    managed=true · phase=hot · action=rollover · step=check-rollover-ready · age 7.3d
```

ILM 서비스는 `RUNNING` 이고 정책도 붙어 있는데 **`check-rollover-ready` 에서 멈춰 있다.**
이유는 간단하다 — filebeat 이 9.5.2 에서 9.5.3 으로 올라가며 **데이터 스트림 이름이
바뀌었고**(`filebeat-9.5.3`), 옛 스트림에는 **쓰는 이가 없다.** ILM 의 rollover 는
쓰기가 있어야 발동하므로 **쓰지 않는 스트림은 영원히 hot 에 머물고 delete 단계에
도달하지 못한다.** 보존 정책이 있는데도 데이터가 안 지워지는 것이다(Gotcha 142).

★ 그래서 "보존 정책을 걸어 두었다" 가 "지워진다" 를 뜻하지 않는다. 판정은
`_ilm/explain` 의 `step` 이 `check-rollover-ready` 에 며칠째 머무는지 보는 것이다.

**③ 검색 엔진이 둘이다.** ES(+Kibana) 와 **Wazuh indexer(OpenSearch 계열)** 가 함께 돈다.

```
elasticsearch-es-default-0   4,146 Mi
kibana                         818 Mi
wazuh-indexer-0              1,024 Mi
                             5,988 Mi  합계
```

이것은 §9-1 의 OpenSearch 행이 "셋째를 들이지 말고 둘을 줄여라" 로 적어 둔 그것이다.

★ **다만 "줄여라" 를 ES 쪽으로 모으라는 뜻으로 읽지 말 것.** Elastic Basic 에는 감사 로그·DLS/FLS·SAML/OIDC 렐름이 **없다**(전부 Enterprise). Wazuh 의 indexer 는 OpenSearch 라 그것들을 무료로 갖는다. 즉 지금의 분업 — ES 가 로그·메트릭(게이팅된 기능이 필요 없는 것), Wazuh indexer 가 보안 이벤트(감사가 필요한 것) — 은 **우연이 아니라 기능 티어와 맞아떨어진다.** 합치려면 어느 쪽으로 합치는지가 곧 기능을 버리는 결정이다: ES 로 모으면 감사·DLS 를 잃고, OpenSearch 로 모으면 §9-1 의 Filebeat 블로커에 걸린다.

**지금 하지 않는 이유**: ②는 17.2 GB 를 지우는 파괴적 작업이라 보존 기간을 먼저
정해야 하고(그 로그를 얼마나 들고 있을 것인지는 우리가 결정한 적이 없다), ①은 ②를
처리하면 샤드·데이터가 줄어 다시 재야 하는 값이다. **순서는 ② → ① → ③ 이다.**

**복귀 조건**: ES 가 다시 재시작을 쌓기 시작하거나 디스크가 압박을 받을 때 —
...
> ★★ **정정 (2026-09-10) — 이 문단의 근거는 Hyper-V 전용이다.**
> "완충 장치가 없다" 는 **H1(동적 메모리 금지)** 에서 나온 결론인데, 그것은
> Hyper-V 제약이다. **WSL 안 KVM 에서는 셋 다 뒤집힌다**(§24 에서 전부 실측):
>
> | | 실측 |
> |---|---|
> | demand paging | Hyper-V 는 게스트가 **3.25 GiB 만 만졌는데 6.02 GiB** 를 잡고 있었다(§24-3) |
> | virtio-balloon | FreeBSD 도 지원한다 — Hyper-V DM 은 불가 |
> | zram 이 아래를 받침 | 2.5 GB 게스트에 스왑 0 → 1332 MB, 클러스터 무영향(§24-7 ⑥) |
>
> 즉 **VM 간 슬랙이 넘어가지 않는다는 전제가 KVM 에서는 성립하지 않는다.**
> 다중 노드를 얇게 만든 주된 이유가 사라진다 — §15-6.
...
> ★★ **정정 (2026-09-10) — 이 산수는 "할당 = 소모" 를 전제한다.**
> Hyper-V 정적 할당에서는 맞지만 **KVM 에서는 만진 만큼만 쓴다.** 실측으로
> OPNsense 는 할당 5.96 GiB 중 **3.25 GiB** 만 만지고 있었다(§24-2).
> 같은 배율이 노드에도 적용되면 56.6 은 상한이지 소요가 아니다.
>
> 그리고 `L0-Target` 종료(+2.8)는 이제 **가정이 아니라 실행 가능한 항목**이다 —
> 그 VM 의 세 역할 중 H5 검증은 일회성 게스트로 빠졌고(§24-5), 표적은 netns 로,
> Zeek 은 파드로 간다(§24-4). **상시 점유가 0 이 된다.**
>
> ★ 다만 아래 문단("DB 복제를 실제로 구성하는 비용이 빠져 있다")은 **그대로
> 유효하다.** 메모리가 풀려도 Patroni·Galera 는 여전히 매니페스트를 새로 쓰는
> 일이다.
...
| 전체 HA 스택 | ⚠ | 56.6 GiB 필요 · 가용 47.6. **단 이 값은 정적 할당 전제다** — KVM 이면 상한이지 소요가 아니다(§11-4 정정 · §15-6) |
...
### 15-6. KVM 다중 노드 — 판단 근거 둘이 바뀌었다 (2026-09-10)

**전략은 그대로다** — 단일 노드에서 먼저 세우는 것이 옳다. 작업량의 대부분
(접속 문자열 34곳 전환 등, §15-1)이 노드 수와 무관하기 때문이다. 바뀐 것은
"그 다음에 무엇이 가능한가" 쪽이다.

#### 낡은 근거 ①  "H5 가 미검증이라 위험하다"

**해소됐다 — 단, KVM 경로에 한해서다.** §24-5 의 일회성 게스트가
`virtio_net` 에서 **PASS 12 · FAIL 0** 을 받았다: BTF · `xt_TPROXY`/`xt_socket`/
`nf_conntrack` · k3s API · **Cilium 정상** · socketLB 경유 200 ·
default-deny 차단과 복귀.

★ 이 결과가 **KVM 다중 노드에는 직접 적용된다**(게스트 NIC 이 곧 virtio_net
이므로). Hyper-V 다중 노드(ADR-051 A안)에는 **유추로만 적용되고 보증하지
않는다** — 그쪽은 `EXPECT_DRV=hv_netvsc` 로 다시 재야 한다.

#### 낡은 근거 ②  "동적 메모리가 금지되어 완충이 없다"

§10-1 이 다중 노드를 "완충 장치가 없는 얇음" 이라 부른 근거는 **H1(Hyper-V 의
동적 메모리 금지)** 이다. KVM 에서는 성립하지 않는다 — demand paging ·
virtio-balloon · 그 아래의 zram. 전부 §24 에서 실측했다.

#### 산수 — 실측 오버헤드 기준

```
워크로드 requests            39.84 GiB   (2026-09-10 실측)
노드 오버헤드 3 × 2.82        8.46       (§10-1-b 실측: DaemonSet 0.92 + 예약 1.9)
WSL 호스트(하이퍼바이저 역할)  ~1.5
                            ───────
장부상 소요                  ~49.8   vs  WSL MemTotal 54.92
```

★ **장부와 물리를 구분할 것**(Gotcha 101·102 가 반복해서 경계하는 자리다).
demand paging 이면 물리 소요는 실사용에 가깝고, 현재 클러스터 실사용은
36.3 GiB 다. 게스트 커널 3개(~3 GiB)를 더해 **물리 ~39~40 GiB** 로 본다.

#### 무엇을 사는가 — §15-2 의 "못 재는 것" 목록에 대고

| §15-2 항목 | KVM 3노드가 답하나 |
|---|:-:|
| 안티어피니티·topologySpread | ✅ 진짜 노드가 3개다 |
| 정족수 상실(노드를 통째로 잃는 것) | ✅ |
| PVC 배치(`local-path`·`WaitForFirstConsumer`) | ✅ 노드마다 다른 디스크 |
| **네트워크 분단·split-brain** | ✅ **오히려 쉽다** — 리눅스 브리지에서 `tc`/`nft` 로 정밀 주입 |
| 지연·타임아웃 정정 | ⚠ 루프백보다 현실적이나 물리 네트워크는 아니다 |
| 자원 경합 성격 | ⚠ 물리 디스크·페이지 캐시는 여전히 공유 |

§15-2 가 *"답해 주지 않는 질문은 '분단되면 어떻게 되는가' 이고 그것이 위험의
대부분"* 이라고 적었는데, **그 항목이 KVM 에서 가장 잘 된다.**

#### ★★ 사지 못하는 것 — 이것은 가용성이 아니다

단일 물리 기기이고, 그 위 **단일 WSL VM 안**이다. WSL 이 죽으면(Gotcha 6)
3노드가 **함께** 죽는다. 그러니 이 구성이 주는 것은 **리허설 환경**이지
가용성이 아니다. 그 구분을 흐리면 §11-3 과 같은 결함("HA 처럼 보이게")을
한 층 위에서 다시 만드는 것이다.

그리고 DB HA 는 여전히 별개다 — §11-4 가 못박은 대로 Patroni·Galera 는
매니페스트를 새로 쓰는 일이고, 진짜 HA 에는 분산 스토리지(Longhorn ~3.5 GiB)가
따로 필요하며 중첩 게스트에서 느리다.

#### 순서

1. §15-1 의 단일 노드 작업을 끝낸다 (변함없다)
2. 그 뒤 **KVM 3노드를 리허설로** 세워 §15-2 의 ✅ 항목 넷을 잰다
3. 그 결과로 §11-5 를 다시 계산한다

★ PVC 27개 재생성은 여전히 따라온다. 리허설은 **별도 클러스터**로 세우고
지금 것을 건드리지 않는 편이 안전하다 — h5-probe 가 그랬듯이.
...
### 22-1. 지금 63.4 GiB 가 어디로 가고 있나 — 실측

```
vmmemWSL (WSL2 VM)          40.9 GiB   ← .wslconfig memory=44GB 상한
L0-OPNsense (Hyper-V)        6.0
L0-Target   (Hyper-V)        2.2
Windows + 데스크톱 앱       ~14.1      ← Memory Compression 1.6 · 편집기 등 포함
                          ───────
                            63.4       (Windows 가용 2.3 GiB)
```
...
MemTotal      43.11 GiB
capacity      45198988Ki
allocatable   42794636Ki   = capacity 의 94.7%
```

★ **오늘의 제약은 호스트가 아니라 `.wslconfig` 의 44GB 상한이다.** 그 상한은
"Windows 데스크톱에 약 7 GB 를 남긴다" 는 판단에서 나왔다(그 파일 주석).

### 22-2. 옮기면 얼마를 버나

호스트 예약 비율은 리눅스 공통이므로 위에서 실측한 **94.7%** 를 그대로 쓴다.

| 구성 | 호스트 OS | L0 랩 | k3s 원시 | allocatable | 지금 대비 |
|---|---:|---:|---:|---:|---:|
| **지금 (WSL2)** | Windows ~14 | 8.4 (Hyper-V) | 43.1 | **40.8** | — |
| 베어 리눅스 · **랩 유지**(KVM) | ~2 | 8.4 | 53.0 | **~50.2** | **+9.4** |
| 베어 리눅스 · **랩 이전** | ~2 | 0 | 61.4 | **~58.2** | **+17.4** |

### 22-3. 그래서 남은 도입이 들어가는가

§7 의 추정 합계는 **8 ~ 14 GiB** 다. 다만 §8-102 에서 OpenFGA 실측이 추정의
**10분의 1**(17Mi)이었으므로 그 표를 그대로 믿을 수는 없다 — 남은 큰 항목이
전부 JVM(Gravitee · Flink · midPoint · Debezium)이라 같은 배율은 아니지만,
현실적인 값은 **8 ~ 11 GiB** 쪽에 가까울 것이다.

| | 여유 | 8 GiB | 11 GiB | 14 GiB |
|---|---:|:-:|:-:|:-:|
| 지금 | +1.2 | ❌ | ❌ | ❌ |
| 베어 · 랩 유지 | +9.4 | ✅ | ⚠ 빠듯 | ❌ |
| 베어 · 랩 이전 | +17.4 | ✅ | ✅ | ✅ |

**답: 된다. 다만 "랩을 어떻게 할 것인가" 가 같이 정해져야 한다.**
랩을 그대로 두면 8~9 GiB 짜리 조합까지이고, 랩을 다른 기기로 옮기면 전부
들어간다.

### 22-4. 메모리 말고 딸려 오는 이득 — 전부 이 레포가 실제로 겪은 것

| 사라지는 것 | 무엇이었나 |
|---|---|
| **Gotcha 6** | WSL 배포판이 유휴 시 종료돼 **파드 110여 개가 전멸**했다. 진단 명령 자체가 원인을 재생산해 한 시간을 썼다(§8-78) |
| **Gotcha 49** | `.wslconfig` 의 `networkingMode=mirrored` 가 k3s 에게 Hyper-V 랩 주소를 고르게 해 **클러스터가 통째로 무너졌다**(§8-77) |
| **Gotcha 55** | 그 잔재가 kine 의 masterlease 로 남아 API 호출의 **절반이 조용히 실패**했다(§8-84) |
| `.wslconfig` 튜닝 전체 | memory · processors · vmIdleTimeout · autoMemoryReclaim · sparseVhd — 전부 이 호스트 고유의 문제였다 |
| **§19-3 ⑤ zram ZSTD** | WSL2 커널은 `[lzo-rle] lzo` **둘뿐이다**(실측). 배포판 기본 커널은 zstd 를 포함하므로 **커널 빌드 없이** 압축비 2.9 → 3.5 로 올라간다 |

★ 위 셋은 전부 **"증상이 원인과 아주 멀었던"** 사고다. 이 호스트 고유의
결함이 사라지는 것은 메모리보다 값이 클 수 있다.

### 22-5. 대가 — 기술만으로 정할 수 없는 것

| | |
|---|---|
| **Windows 데스크톱이 사라진다** | §21-5 가 이미 못박았다 — "이 호스트는 사용자의 작업 기기이기도 하다. 기술적 판단만으로 정할 사안이 아니다" |
| **L0 랩을 KVM/libvirt 로 옮겨야 한다** | OPNsense 는 KVM 에서 잘 돈다. 다만 §8-51 의 **포트 미러링 기반 Zeek** 구성은 Hyper-V 가상 스위치 전제라 다시 짜야 한다 |
| **재구축 비용** | `bootstrap-wsl-k3s.sh` 에 베어메탈 변종이 필요하다. `local/` 의 나머지(install-operators · install-argocd · create-secrets · build-images)는 대부분 그대로 쓸 수 있다 |
| **풀 HA 는 여전히 안 된다** | §11-4 의 56.6 GiB 는 **3노드**를 전제하고, 단일 물리 기기에서 3노드는 다시 VM 이라 게스트 OS 몫이 돌아온다. 이 이야기는 "남은 도입" 과 별개다 |

### 22-6. 판단

**남은 도입(8~14 GiB)이 목적이라면 베어메탈 리눅스가 답이다** — Proxmox(+5.0)
보다 크고(+9.4 ~ +17.4), 하이퍼바이저 게스트 몫이 없으며, 이 호스트 고유의
사고 세 가지가 함께 사라진다.
...
## 24. L0 랩을 WSL 안 KVM 으로 옮겼다 (2026-09-10, 완료)

> §23 이 "메모리가 있는 곳" 을 물었다면 이것은 그 답을 랩에 적용한 것이다.
> **끝났다** — OPNsense·표적·Zeek·H5 가 전부 옮겨졌고(24-5·24-6·24-6-b),
> systemd 유닛이 재부팅을 넘기며(24-9), Hyper-V 자산은 지웠다(24-10).
> 랩이 6.85~8.85 GiB(Windows 선할당) → **~2.7 GiB(WSL, 만진 만큼)** 가 됐고,
> 5일째 죽어 있던 파이프라인이 살아났다.
...
쓰므로 Hyper-V 쪽 VM 과 체크포인트 2개가 그대로 남아 롤백 수단이 되었다.

> ★★ **2026-09-10 에 그 Hyper-V 자산을 지웠다(§24-10) — 이제 롤백 수단은 없다.**
> 다만 OPNsense 상태는 `/var/lib/l0/opnsense.qcow2` 에 그대로이고 지금 그것으로
> 돌고 있다.
...
> ★ **근원은 §24-10 에서 사라졌다** — `vEthernet (L0-LAN)` 어댑터를 지우자
> Windows 의 DNS 목록에서 `10.77.0.1` 이 빠졌다(남은 것은 Wi-Fi 의
> `192.168.1.1` 과 이더넷의 `168.126.63.x`). 그래도 **고정은 풀지 않았다** —
> 랩이 아니어도 Windows 어댑터 변동이 클러스터 DNS 를 흔드는 구조 자체가
> 위험하기 때문이다.
> ★★ **대가는 있다**: 노드가 더 이상 호스트의 DNS 를 따라가지 않으므로,
> `1.1.1.1`·`8.8.8.8` 이 막힌 망으로 옮기면 DNS 가 끊긴다. 그때는 위 되돌리기로.
...
### 24-10. Hyper-V 자산을 지웠다 (2026-09-10)

KVM 쪽이 검증을 전부 통과한 뒤 정리했다. **되돌릴 수 없는 작업이라 무엇을
잃는지 먼저 적는다.**

| 지운 것 | |
|---|---|
| VM 2대 | `L0-OPNsense`(체크포인트 2개 포함) · `L0-Target` |
| 디스크 5개 | flat 7.94 · OPNsense 7.12(병합 후) · Target 6.29 GB 등 — **21.35 GB 회수** |
| 가상 스위치 2개 | `L0-LAN`(Internal) · `L0-WAN`(External, Wi-Fi 바인딩) |
| **남긴 것** | SSH 키 3개 · `L0-Target-seed.iso` — `L0-OPNsense-key` 는 **지금도 KVM 게스트 접속에 쓴다** |

★ `Remove-VM` 이 체크포인트를 **먼저 병합**한다 — 그래서 `L0-OPNsense.vhdx` 가
2.65 → 7.12 GB 로 커진 뒤 지워졌고 `.avhdx` 둘은 그 시점에 이미 없었다.
(체인이 걱정된다면 이 순서 자체가 안전장치다.)

#### 무엇을 잃었나

**롤백 수단이다.** 다만 OPNsense 상태는 `/var/lib/l0/opnsense.qcow2`(평탄화
사본)에 그대로 있고 지금 그것으로 돌고 있으며, `L0-Target` 은 세 역할이 전부
대체됐다(§24-6-b) — 필요하면 `prepare-target-vm.sh` 로 다시 만든다.

#### ★★ 덤으로 DNS 지뢰의 근원이 사라졌다

`vEthernet (L0-LAN)` 어댑터가 없어지자 Windows 의 DNS 목록에서 **`10.77.0.1` 이
빠졌다.** §24-8 의 사슬(랩 OPNsense → WSL 프록시 → CoreDNS → 파드 전부)이
구조적으로 성립하지 않게 된 것이다.

★ 그래도 노드 DNS 고정(`chattr +i`)은 **풀지 않았다** — 랩이 아니어도 Windows
어댑터 변동이 클러스터 DNS 를 흔드는 구조 자체가 위험하다.

#### 검증 — External 스위치를 뗐으니 물리 NIC 을 확인해야 한다

`L0-WAN` 은 Wi-Fi 어댑터에 바인딩돼 있어 제거 시 순단 위험이 있다. 제거 뒤 실측:

```
Windows   Wi-Fi Up(150 Mbps) · 1.1.1.1:443 OK · DNS 조회 OK
          DNS 목록에 10.77.0.1 없음  ← 근원 제거 확인
노드      resolv.conf 1.1.1.1/8.8.8.8 · 조회 OK
랩        l0-lab-net·l0-opnsense active · 표적→8.8.8.8 OK · OPNsense SSH OK
클러스터   130 Running · ArgoCD Synced/Healthy · 클러스터 외부 DNS OK
Zeek      shipper 정상
```

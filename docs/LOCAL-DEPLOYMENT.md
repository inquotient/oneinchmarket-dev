...
- **백업이 전무하다**(ADR-018 `Open`). §13-5 의 결론 — 물리 디스크가 한 장인 이 호스트에서는 **복제보다 백업이 값어치가 크다**. 그리고 MinIO 에만 넣으면 디스크 장애를 못 막는다. **호스트 밖 반출**이 있어야 의미가 생긴다
...
- **ADR-015 의 실제 선택지는 ⓒ(Longhorn/OpenEBS) 하나다**(§11-2-b). Hyper-V 공유 VHDX 는 저장소가 공유를 지원하지 않아 부착이 거부되고, Storage Spaces 미러는 물리 디스크가 하나라 불가하다. 판단 기준은 "메모리 3.5 GiB + 쓰기 증폭을 **이동성**과 바꿀 것인가" 이며 **내구성은 어느 쪽으로도 얻지 못한다**
- 물리 디스크를 한 장 더 꽂는 것이 이 호스트에서 바꿀 수 있는 유일한 전제다
...
### 11-1. 제어평면 HA — 산술적으로는 된다

k3s 는 내장 etcd 로 다중 server 를 지원한다(`--cluster-init` + `--server`).
etcd 정족수는 3이고 §10-1-b 의 예산에 3노드가 들어가므로 **구성 자체는 가능**하다.

그러나 **한 대의 물리 호스트 위의 etcd 3중화는 정족수 연극이다.** 호스트가
죽으면 셋이 같이 죽는다. 디스크도 하나이고 전원도 하나다. 게다가 etcd 는
fsync 민감한데 같은 NVMe 에 PVC 27개의 I/O 가 함께 얹힌다 — **3중화가 오히려
지연을 늘린다.**

견디는 장애는 좁다: VM 커널 패닉, 노드 드레인, k3s 프로세스 사망. 넓은 장애
(호스트·디스크·전원)는 하나도 못 견딘다.
...
#### ③ 그리고 이것이 핵심 — 디스크가 하나면 어떤 계층도 내구성을 못 준다

이 부분이 §11-1 의 "정족수 연극" 과 같은 구조다.

> **이동성(mobility) ≠ 내구성(durability).**
>
> 어떤 스토리지 가상화를 얹어도 복제본은 **전부 같은 NVMe 한 장**에 떨어진다.
> 그 디스크가 죽으면 3중 복제도 같이 죽는다. 늘어나는 것은 "노드가 빠져도
> 볼륨이 따라간다" 는 성질이지 "디스크가 죽어도 산다" 가 아니다.
...
| 어떤 방법으로든 **내구성** | ❌ 디스크가 하나다 |

즉 **"Hyper-V 로 스토리지 가상화" 는 안 되고, "k8s 안에서 복제" 는 된다.**
그리고 §12-3 의 장애 표에서 실제로 필요한 것은 이동성이다 — 노드 드레인,
VM 크래시, 롤링 업그레이드를 견디게 해 준다. 물리 디스크 장애는 이 호스트에서
어차피 못 견디므로, **ADR-015 를 닫을 때의 실제 선택지는 ⓒ(Longhorn/OpenEBS)
하나**이고 판단 기준은 "메모리 3.5 GiB + 쓰기 증폭을 이동성과 바꿀 것인가" 다.

> 물리 디스크를 한 장 더 꽂으면 그림이 달라진다. Storage Spaces 미러 또는
> Longhorn 의 복제본을 **다른 물리 디스크**에 배치할 수 있게 되어 그때 비로소
> 내구성이 생긴다. 지금 구성에서 바꿀 수 있는 유일한 전제이기도 하다.
...
| 층 | 가능한가 | 단서 |
|---|---|---|
| 제어평면 3중화 | ✅ 예산 안에 든다 | 단일 호스트라 **정족수 연극**. 검증용으로만 의미 |
| 무상태 워크로드 복제 | ✅ 지금도 된다 | nginx·logstash 등 |
| **상태 있는 워크로드** | ⚠ | Longhorn 등으로 **이동성은 얻을 수 있다**(§11-2-b). 그러나 **DB 복제가 미구성**이라 DB 3종은 그래도 단일이다 |
| 하이퍼바이저 스토리지 가상화 | ❌ | 공유 VHDX 부착 거부 · 물리 디스크 1개(§11-2-b) |
| 전체 HA 스택 | ❌ | 56.6 GiB 필요 · 가용 47.6 |

**결론 — "HA 처럼 보이게" 는 오늘도 되고, "HA" 는 안 된다.** 그리고 전자가
위험하다. §11-3 의 결함이 정확히 그 형태다.
...
- **스토리지 이동성** — `local-path` 그대로면 PVC 는 노드에 고정되고, 노드가 죽으면
  그 위의 PVC 파드는 다른 노드에서 뜨지 못한다. **Longhorn 등을 얹으면 이동성은
  생긴다**(§11-2-b) — 다만 내구성은 아니다. 물리 디스크가 한 장이다
...
### 13-5. ★ 그러나 이 호스트에서는 복제보다 백업이 값어치가 크다

정직하게 적어 둔다.

> **복제는 백업이 아니다.** 복제본은 `DROP TABLE` 을 **충실하게 복제한다.**
> 그리고 이 호스트는 **물리 디스크가 한 장**이다(§11-2-b) — 복제본 셋이 전부
> 같은 NVMe 에 있으므로 **디스크가 죽으면 복제도 같이 죽는다.**

즉 이 호스트에서 복제가 막아 주는 것은 **노드/VM 장애**뿐이고, 그것은 단일
호스트에서 그리 흔한 사고가 아니다. 반면 막지 못하는 것은 실수(논리 손상)와
디스크 장애이며 **둘 다 실제로 데이터를 잃는다.**

ADR-018(백업)이 `Open` 이고 현재 **백업이 전무**하다(실측: `pg_dump`·
`mysqldump`·`mongodump` 어느 것도 없다). 비용 대비로 보면:

| 조치 | 메모리 | 막아 주는 것 |
|---|---:|---|
| 논리 덤프 CronJob → MinIO | ~0 | 논리 손상(실수) |
| **위 + 호스트 밖으로 반출** | ~0 | **논리 손상 + 디스크 장애** |
| DB 복제 5종 | 5.07 GiB | 노드/VM 장애만 |

**MinIO 에만 넣는 것으로는 디스크 장애를 못 막는다** — MinIO 도 같은 NVMe 에
있다(ADR-018 이 지적한 "MinIO 를 백업 대상으로 쓰는 것은 순환" 과 별개의,
더 단순한 이유다). 호스트 밖(다른 물리 디스크·NAS·오브젝트 스토리지)으로
나가야 의미가 생긴다.

**권고: §13-1(지혈) → 백업 CronJob + 호스트 밖 반출 → 그 다음 §13-2 의 복제.**
복제를 하지 말자는 것이 아니라 **순서가 그렇다**는 것이다.
